...
func (r *ClusterPodPlacementConfigReconciler) getCorrectHostmountAnyUIDSCC(ctx context.Context) (string, error) {
	log := ctrllog.FromContext(ctx)
	...
	if minor < 32 {
		return "hostmount-anyuid", nil
	}

	return "hostmount-anyuid-v2", nil
}
...
func (r *ClusterPodPlacementConfigReconciler) reconcile(ctx context.Context, clusterPodPlacementConfig *multiarchv1beta1.ClusterPodPlacementConfig) error {
	...
	requiredSCCHostmountAnyUID, err := r.getCorrectHostmountAnyUIDSCC(ctx)
	if err != nil {
	...
	objects := []client.Object{
		...
		buildControllerDeployment(clusterPodPlacementConfig, requiredSCCHostmountAnyUID),
