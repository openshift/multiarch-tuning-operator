	...
	When("The node affinity scoring plugin is enabled", func() {
		Context("with different types of preferred affinities", func() {
			...
			AfterEach(func() {
				...
				Eventually(ValidateDeletion(k8sClient, ctx)).Should(Succeed(), "the ClusterPodPlacementConfig should be deleted")
			})
			...
			It("skips appending the ClusterPodPlacementConfig", func() {
				preferredSchedulingTerm := corev1.PreferredSchedulingTerm{
					Weight: int32(10),
					Preference: corev1.NodeSelectorTerm{
						MatchExpressions: []corev1.NodeSelectorRequirement{
