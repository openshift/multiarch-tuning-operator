...
var _ = Describe("The Pod Placement Operand", func() {
	...
	Context("with different types of preferred affinities", func() {
		...
		It("does not append any ClusterPodPlacementConfig node affinity when plugin is disabled", func() {
			var err error
			By("Get the v1beta1 version of the CPPC")
			ppc := &v1beta1.ClusterPodPlacementConfig{}
			err = client.Get(ctx, runtimeclient.ObjectKeyFromObject(&v1beta1.ClusterPodPlacementConfig{
			...
			// set the plugins to nil
			if ppc.Spec.Plugins.NodeAffinityScoring.IsEnabled() {
				ppc.Spec.Plugins.NodeAffinityScoring.Enabled = false
			}
			...
			Expect(ppc.Spec.Plugins.NodeAffinityScoring.IsEnabled()).To(BeFalse())
